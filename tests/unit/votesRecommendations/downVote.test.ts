...
describe("Counts a down vote", () => {
  ...
  it("Should check the score to delete the most negative recommendations", async () => {
    ...
    jest
      .spyOn(recommendationRepository, "find")
      .mockResolvedValueOnce(negativeRecommendation);
  ...
  it("Shouldn't increment the vote for a non-existent recommendation", async () => {
    ...
    await expect(recommendationService.getById(randomId)).rejects.toEqual(
      notFoundError()
    );
    await expect(recommendationService.downvote(randomId)).rejects.toEqual(
      notFoundError()
    );
